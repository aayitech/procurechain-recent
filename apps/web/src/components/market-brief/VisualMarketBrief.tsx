...
type BriefSignal = { key: string; name: string; value: number; unit: string; change: number | null; history: Array<{ asOf: string; price: number }>; href: string };
...
export function VisualMarketBrief() {
  ...
  const signals = useMemo<BriefSignal[]>(() => {
    if (!data) return [];
    return [
      ...data.commodities.map((item) => ({ key: item.symbol, name: item.name, value: item.latestPrice, unit: item.unit, change: item.change30d, history: item.sparkline, href: `/market-intelligence/commodity/${item.symbol}` })),
      ...data.fx.map((item) => ({ key: item.quoteCode, name: `${item.baseCode}/${item.quoteCode}`, value: item.latestRate, unit: '', change: item.change30d, history: item.sparkline, href: `/market-intelligence/fx/${item.quoteCode}` })),
