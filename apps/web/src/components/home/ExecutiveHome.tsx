...
import { ArrowRight, BarChart3, Bot, Boxes, Building2, Droplets, Factory, Globe2, Leaf, Newspaper, Package, TrendingDown, TrendingUp } from 'lucide-react';
...
export function ExecutiveHome() {
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <div className="container-page space-y-8 py-8">
      {isLoading && <div className="h-36 animate-pulse rounded-2xl border border-slate-800 bg-slate-900" />}
      {signals.length > 0 && <section><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-blue-400">Today&apos;s key signals</p><h2 className="mt-1 text-xl font-semibold">Markets moving now</h2></div><Link href="/market-intelligence" className="flex items-center gap-1 text-sm font-medium text-blue-400 hover:text-blue-300">Open terminal <ArrowRight className="h-4 w-4" /></Link></div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{signals.slice(0, 5).map((item) => { const positive = (item.change ?? 0) >= 0; return <Link key={`${item.category}-${item.id}`} href={item.href} className="group rounded-xl border border-slate-800 bg-slate-900/80 p-4 transition hover:border-blue-500/60"><div className="flex items-start justify-between gap-2"><p className="truncate text-sm font-semibold text-slate-200">{item.label}</p>{positive ? <TrendingUp className="h-4 w-4 text-emerald-400" /> : <TrendingDown className="h-4 w-4 text-rose-400" />}</div><p className="mt-3 text-xl font-bold">{number(item.value)} <span className="text-xs font-normal text-slate-500">{item.unit}</span></p><p className={`mt-1 text-sm font-semibold ${positive ? 'text-emerald-400' : 'text-rose-400'}`}>{item.change == null ? 'Current' : `${positive ? '+' : ''}${item.change.toFixed(2)}% (30D)`}</p>{item.history.length > 1 && <div className="mt-3"><Sparkline data={item.history} positive={positive} /></div>}</Link>; })}</div>
      </section>}
      {(stories.length > 0 || signals.length > 0) && <section className={`grid gap-5 ${stories.length > 0 && signals.length > 0 ? 'lg:grid-cols-[1.7fr_1fr]' : ''}`}>
        {stories.length > 0 && <div className="rounded-2xl border border-slate-800 bg-slate-900/65 p-5"><div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 text-lg font-semibold"><Newspaper className="h-5 w-5 text-blue-400" /> Today&apos;s top stories</h2><Link href="/news" className="text-xs font-medium text-blue-400">View all</Link></div><div className="grid gap-4 md:grid-cols-3">{stories.map((story) => <a key={story.link} href={story.link} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-xl border border-slate-800 bg-slate-950/65">{story.imageUrl && <div className="h-28 bg-cover bg-center" style={{ backgroundImage: `url(${story.imageUrl})` }} />}<div className="p-4"><p className="line-clamp-3 text-sm font-semibold leading-5 group-hover:text-blue-300">{story.title}</p><p className="mt-3 text-xs text-slate-500">{story.source}</p></div></a>)}</div></div>}
        {signals.length > 0 && <div className="rounded-2xl border border-slate-800 bg-slate-900/65 p-5"><h2 className="flex items-center gap-2 text-lg font-semibold"><BarChart3 className="h-5 w-5 text-blue-400" /> Market watch</h2><div className="mt-4 divide-y divide-slate-800">{signals.slice(0, 6).map((item) => <Link key={`watch-${item.category}-${item.id}`} href={item.href} className="flex items-center justify-between gap-3 py-3 text-sm hover:text-blue-300"><span className="truncate text-slate-300">{item.label}</span><span className={`font-semibold ${(item.change ?? 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{item.change == null ? number(item.value) : `${item.change >= 0 ? '+' : ''}${item.change.toFixed(2)}%`}</span></Link>)}</div></div>}
      </section>}
