...
export function ExecutiveHome() {
  ...
  const signals = useMemo<Signal[]>(() => {
    ...
    return [
      ...
      ...data.fx.map((item) => ({ id: `${item.baseCode}-${item.quoteCode}`, label: `${item.baseCode}/${item.quoteCode}`, value: item.latestRate, unit: '', change: item.change30d, history: item.sparkline, href: `/market-intelligence?instrument=${encodeURIComponent(`fx:${item.baseCode}:${item.quoteCode}`)}`, category: 'fx' })),
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <div id="dashboard" className="container-page space-y-8 py-8">
      ...
      {!isLoading && <section aria-label="Personal market overview" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <OverviewStat icon={Activity} label="Market coverage" value={`${signals.length} live`} detail="Verified instruments" />
