...
// Real, publicly documented coordinates of major global shipping
// chokepoints and container ports — used to pull live weather conditions
// relevant to procurement/logistics risk (port operations, canal transits).
export const TRADE_ROUTE_POINTS = [
  { id: 'SUEZ_CANAL', name: 'Suez Canal', region: 'Egypt', lat: 30.5852, lon: 32.2654, kind: 'canal' },
  { id: 'PANAMA_CANAL', name: 'Panama Canal', region: 'Panama', lat: 9.0820, lon: -79.7674, kind: 'canal' },
...
export type TradeRoutePointId = (typeof TRADE_ROUTE_POINTS)[number]['id'];
...
export class OpenMeteoProvider {
  ...
  async fetchAll(): Promise<PortCondition[]> {
    const results = await Promise.allSettled(TRADE_ROUTE_POINTS.map((point) => this.fetchOne(point)));
    return results.map((result, i) =>
      result.status === 'fulfilled' ? result.value : this.unavailable(TRADE_ROUTE_POINTS[i]),
    );
  }

  private async fetchOne(point: (typeof TRADE_ROUTE_POINTS)[number]): Promise<PortCondition> {
    ...
    if (!response.ok) {
      this.logger.warn(`Open-Meteo request failed for ${point.id}: ${response.status}`);
      return this.unavailable(point);
    }
    ...
    if (!current) return this.unavailable(point);
  ...
  private unavailable(point: (typeof TRADE_ROUTE_POINTS)[number]): PortCondition {
    return {
      id: point.id,
      name: point.name,
      region: point.region,
